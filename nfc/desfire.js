...
module.exports = function desfire(config, transmit, printLog, printOut) {
    ...
    return transmit(wrap(0x5A, appId), 40)
        .then((data) => {
            printLog(`res: ${data.toString('hex')}`);
            if (data[0] !== 0x91) {
                return false;
            }

            printLog(`out: ${wrap(0x1A, keyId).toString('hex')}`);
        ...
        .then((data) => {
            ...
            printLog(`out: ${wrap(0xAF, msg).toString('hex')}`);
            return Promise.all([
        ...
        .then(([data, RndA, msg]) => {
            ...
            printLog(`out: ${wrap(0xBD, fileId).toString('hex')}`);
        ...
        .then((data) => {
            ...
            printOut(data.slice(0, 15).toString().replace(/\D+/g, ''));
            return true;
        });
};
